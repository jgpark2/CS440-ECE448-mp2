			...
			Board* paraDrop_board = new Board(board);
 			paraDrop_board->parent = &board;
  			board.children.push_back(paraDrop_board);
   			paraDrop_board->makeMove(*it, 0, 0, 1.0);
   			//paraDrop_board->makeMove(paraDrop_board->findHighestUnclaimedSquare(), 0, 0, 1.0);
   			int test_val = minimax(*paraDrop_board, depth-1, false);
   			//cout << "Comparing " << bestValueMax << " and " << test_val << endl;
 			bestValueMax = max(bestValueMax, test_val);
 			//cout << "test_val takes value of " << test_val << endl;
		}
		return bestValueMax;
	}
	else
	{
		...
		for(vector<int>::const_iterator it = unoccupied_squares.begin(); it!=unoccupied_squares.end(); ++it)
		{
			Board* paraDrop_board = new Board(board);
 			paraDrop_board->parent = &board;
  			board.children.push_back(paraDrop_board);
   			paraDrop_board->makeMove(*it, 1, 0, 1.0);
   	   		//paraDrop_board->makeMove(paraDrop_board->findHighestUnclaimedSquare(), 1, 0, 1.0);
   			int test_val = minimax(*paraDrop_board, depth-1, true);
 			bestValueMin = min(bestValueMin, test_val);
		}
		return bestValueMin;	
	}
}
...
vector<int> minimax(Board board, int depth, bool maximizingPlayer, double gamma, int nodes_expanded, bool sabotage)
{
	...
	if(maximizingPlayer)
	{
		...
		if(maxNeighbors.empty())
		{
			...
			bestValue = max(bestValue, test_val);
...
int main(int argc, char* argv[])
{
	...
	d->addPlayer("Green");
	d->printBoard();
	cout << "Index of highest unclaimed square: " << d->findHighestUnclaimedSquare() << endl;
	d->printScores();
	cout << "Running minimax" << endl;
	int minimax_heuristic = minimax(*d, 3, true);
